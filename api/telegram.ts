...
function getScoringEmoji(scoring: "A" | "B" | "C"): string {
  switch (scoring) {
    case "A":
      return "🔥";
    case "B":
...
export async function sendEstimateToTelegram(data: {
  ...
  const featuresText = data.features && data.features.length > 0 ? data.features.map((feature) => featureLabels[feature] || feature).join(", ") : "Не выбраны";
